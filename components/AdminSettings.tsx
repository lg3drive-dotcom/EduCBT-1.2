...
const AdminSettings: React.FC<AdminSettingsProps> = ({ settings, questions, onUpdateSettings, onImportQuestions, onReset }) => {
  ...
  const [backupSubject, setBackupSubject] = useState<string>('');
  ...
  const handleBackup = () => {
    const dataToExport = !backupSubject || backupSubject === 'SEMUA'
      ? questions 
      : questions.filter(q => q.subject.toLowerCase().includes(backupSubject.toLowerCase()));
    ...
    link.href = url;
    link.download = `EduCBT_Backup_${backupSubject || 'Semua'}_${timestamp}.json`;
    link.click();
        ...
        <div className="space-y-6">
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
            ...
            <div className="flex gap-2">
              <input 
                type="text" 
                placeholder="Mapel/Semua" 
                value={backupSubject} 
                onChange={(e) => setBackupSubject(e.target.value)}
                className="flex-1 px-3 py-2 bg-white border rounded-xl text-xs font-bold outline-none"
              />
